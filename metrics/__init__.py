...
from metrics.mcp import (
    fulledge_total as mcp_fulledge_total,
    edgefeat_total as mcp_edgefeat_total,
    node_total as mcp_node_total,
)
from metrics.mis import (
    fulledge_total as mis_fulledge_total,
    edgefeat_total as mis_edgefeat_total,
    node_total as mis_node_total,
)
...
def get_trainval_fulledge_metric(problem):
    if problem == "tsp":
        ...
        raise NotImplementedError(
            f"Train/val metric for fulledge problem {problem} has not been implemented."
        )
...
def get_test_fulledge_metric(problem):
    if problem in MCP_VARIANTS:
        ...
        raise NotImplementedError(
            f"Test metric for fulledge problem {problem} has not been implemented."
        )
...
def get_trainval_edgefeat_metric(problem):
    if problem == "tsp":
        ...
        raise NotImplementedError(
            f"Train/val metric for edge problem {problem} has not been implemented."
        )
...
def get_test_edgefeat_metric(problem):
    if problem in MCP_VARIANTS:
        ...
        raise NotImplementedError(
            f"Test metric for edge problem {problem} has not been implemented."
        )
...
def get_trainval_node_metric(problem):
    if problem in MCP_VARIANTS:
        ...
        return node_compute_f1
    raise NotImplementedError(
        f"Train/val metric for node problem {problem} has not been implemented."
    )


def get_test_node_metric(problem):
    if problem in MCP_VARIANTS:
        return mcp_node_total
    elif problem == "mis":
        ...
        return node_compute_f1
    raise NotImplementedError(
        f"Test metric for node problem {problem} has not been implemented."
    )
    ...
    if embed == "edge":
        if eval == "edge":
            if problem == "tsp":
                ...
                raise NotImplementedError(
                    f"Preprocessing for {embed=}, {eval=}, {problem=} not implemented"
                )
        elif eval == "fulledge":
            ...
            raise NotImplementedError(
                f"Unknown eval '{eval}' for embedding type 'edge'."
            )
    elif embed == "node":
        if eval == "node":
...
def assemble_metric_function(
    preprocess_function, eval_function, preprocess_additional_args=None
):
    if preprocess_additional_args is None:
        preprocess_additional_args = {}

    def final_function(raw_scores, target, **kwargs):
        l_inferred, l_targets, l_adjacency = preprocess_function(
            raw_scores, target, **kwargs, **preprocess_additional_args
        )
        try:  # We try to add the list of adjacencies to the eval function
            ...
            if (
                str_error == "takes 2 positional arguments but 3 were given"
            ):  # The eval function doesn't handle the adjacencies (OLD functions)
...
def setup_trainval_metric(
    pl_model: GNN_Abstract_Base_Class, config: dict, soft=True
) -> None:
    ...
    try:
        ...
        metric_fn = assemble_metric_function(
            preprocess_function=preprocess_function,
            eval_function=eval_fn,
            preprocess_additional_args=preprocess_additional_args,
        )
        ...
        if not soft:
            raise ne
        print(
            f"There was a problem with the train_val setup metric. I'll let it go anyways, but additional metrics won't be saved. Error stated is: {ne}"
        )
...
def setup_metric(
    pl_model: GNN_Abstract_Base_Class, config: dict, soft=False, istest=False
) -> None:
    """
