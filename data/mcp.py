...
class MCP_Generator(Base_Generator):
    ...
    def __init__(self, name, args):
        ...
        subfolder_name = "MCP_{}_{}_{}_{}".format(
            num_examples, self.n_vertices, self.clique_size, self.edge_density
        )
        path_dataset = os.path.join(args["path_dataset"], "mcp", subfolder_name)
    ...
    def _plant_clique(self, W):
        assert isinstance(
            self.clique_size, int
        ), f"Clique size is not an int {self.clique_size=}"
    ...
    def _solution_conversion(cls, target, dgl_graph):
        ...
        node_classif = (target.sum(dim=-1) != 0).to(
            target.dtype
        )  # Get a node classification of shape (N)
    ...
    def mcp_adj_to_ind(adj) -> list:
        ...
        if len(adj.shape) == 2:
            ...
            adj = adj.unsqueeze(0)
        bs, _, _ = adj.shape
        sol_onehot = torch.sum(
            adj, dim=-1
        )  # Gets the onehot encoding of the solution clique
        l_sol_indices = [
            torch.where(sol_onehot[i] != 0)[0] for i in range(bs)
        ]  # Converts the onehot encoding to a list of the nodes' numbers
...
class MCP_Generator_True(Base_Generator):
    ...
    def __init__(self, name, args):
        ...
        self.n_vertices = args["n_vertices"]
        subfolder_name = "MCPTrue_{}_{}_{}_{}".format(
            num_examples, self.n_vertices, self.clique_size, self.edge_density
        )
        path_dataset = os.path.join(args["path_dataset"], "mcptrue", subfolder_name)
        super().__init__(name, path_dataset, num_examples)
        self.data = []
        self.constant_n_vertices = True
        utils.check_dir(self.path_dataset)
    ...
    def create_dataset(self):
        ...
        solver.solve()
        assert (
            len(solver.solutions) == self.num_examples
        ), "Error somewhere in MCP solver."
        ...
        l_k = [
            MCP_Generator.mcp_ind_to_adj(elt, self.n_vertices)
            for elt in clique_solutions
        ]
        l_k = [
            self._solution_conversion(K.to(int), connectivity_to_dgl(B))
            for (B, K) in zip(l_b, l_k)
        ]
...
class MCP_Generator_NX(Base_Generator):
    """
    Generator for the Maximum Clique Problem.
    This generator finds the max clique
    """
    ...
    def __init__(self, name, args):
        ...
        self.num_examples = num_examples
        self.n_vertices = args["n_vertices"]
        subfolder_name = "MCPNX_{}_{}_{}_{}".format(
            num_examples, self.n_vertices, self.clique_size, self.edge_density
        )
        path_dataset = os.path.join(args["path_dataset"], "mcpnx", subfolder_name)
        super().__init__(name, path_dataset, num_examples)
        self.data = []
        self.constant_n_vertices = True
        utils.check_dir(self.path_dataset)
